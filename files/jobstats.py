...
class Jobstats:
    ...
    def get_job_stats(self, *args):
        ...
        if not args or "total_memory" in args:
            self.get_data('total_memory', "max_over_time(cgroup_memory_total_bytes{{slurm_cluster='{cluster}',jobid='{jobid}',step='',task=''}}[{diff}s])", True)
        if not args or "used_memory" in args:
            self.get_data('used_memory', "max_over_time(((cgroup_memory_rss_bytes{{slurm_cluster='{cluster}',jobid='{jobid}',step='',task=''}}+cgroup_memory_shmem_bytes{{slurm_cluster='{cluster}',jobid='{jobid}',step='',task=''}}) or 1*cgroup_memory_rss_bytes{{slurm_cluster='{cluster}',jobid='{jobid}',step='',task=''}})[{diff}s:])", True)
        if not args or "total_time" in args:
            self.get_data('total_time', "max_over_time(cgroup_cpu_total_seconds{{slurm_cluster='{cluster}',jobid='{jobid}',step='',task=''}}[{diff}s])", True)
        if not args or "cpus" in args:
            self.get_data('cpus', "max_over_time(cgroup_cpus{{slurm_cluster='{cluster}',jobid='{jobid}',step='',task=''}}[{diff}s])", True)

        # and now GPUs
        if self.gpus:
            if not args or "gpu_total_memory" in args:
                if c.GPU_EXPORTER_JOBID:
                    self.get_data('gpu_total_memory', "max_over_time(nvidia_gpu_memory_total_bytes{{slurm_cluster='{cluster}',jobid='{jobid}'}}[{diff}s:])")
                else:
                    self.get_data('gpu_total_memory', "max_over_time((nvidia_gpu_memory_total_bytes{{slurm_cluster='{cluster}'}} and nvidia_gpu_jobId == {jobid})[{diff}s:])")
            if not args or "gpu_used_memory" in args:
                if c.GPU_EXPORTER_JOBID:
                    self.get_data('gpu_used_memory', "max_over_time(nvidia_gpu_memory_used_bytes{{slurm_cluster='{cluster}',jobid='{jobid}'}}[{diff}s:])")
                else:
                    self.get_data('gpu_used_memory', "max_over_time((nvidia_gpu_memory_used_bytes{{slurm_cluster='{cluster}'}} and nvidia_gpu_jobId == {jobid})[{diff}s:])")
            if not args or "gpu_utilization" in args:
                if c.GPU_EXPORTER_JOBID:
                    self.get_data('gpu_utilization', "avg_over_time((nvidia_gpu_duty_cycle{{slurm_cluster='{cluster}',jobid='{jobid}'}} or (nvidia_gpu_graphics_util_percent{{slurm_cluster='{cluster}',jobid='{jobid}'}} * 100))[{diff}s:])")
                else:
                    self.get_data('gpu_utilization', "avg_over_time(((nvidia_gpu_duty_cycle{{slurm_cluster='{cluster}'}} or (nvidia_gpu_graphics_util_percent{{slurm_cluster='{cluster}'}} * 100)) and nvidia_gpu_jobId == {jobid})[{diff}s:])")
