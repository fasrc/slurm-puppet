...
class Jobstats:
    ...
    def get_job_stats(self, *args):
        ...
        if not args or "used_memory" in args:
            self.get_data('used_memory', "max_over_time(((cgroup_memory_rss_bytes{{slurm_cluster='{cluster}',jobid='{jobid}',step='',task=''}}+cgroup_memory_shmem_bytes{{cluster='{cluster}',jobid='{jobid}',step='',task=''}}) or 1*cgroup_memory_rss_bytes{{cluster='{cluster}',jobid='{jobid}',step='',task=''}})[{diff}s:])", True)
